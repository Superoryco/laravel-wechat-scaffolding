...
namespace App\Library\Readability;

use App\Library\Readability\Nodes\DOM\DOMDocument;
use App\Library\Readability\Nodes\DOM\DOMElement;
use App\Library\Readability\Nodes\DOM\DOMNode;
use App\Library\Readability\Nodes\DOM\DOMText;
use App\Library\Readability\Nodes\NodeUtility;
...
//                        throw new ParseException('Could not parse text.');
    ...
    private function getArticleTitle()
    {
        ...
        $originalTitle = $this -> removeEnter($originalTitle);
        $originalTitle = $this -> replaceAllDash($originalTitle);
        ...
        if (preg_match('/ [\|\-\\\\\/>»«<] /i', $curTitle)) {
            ...
            if (!$match) {
                ...
                if ($this -> wordCount($curTitle) < 3) {
                    ...
                    $this->logger->info(sprintf('[Metadata] Title too short, using the first part of the title instead: \'%s\'', $curTitle));
                } elseif ($this -> wordCount(substr($curTitle, 0, strpos($curTitle, ':'))) > 5) {
         ...
         * the original title.
         */
        $curTitleWordCount = $this -> wordCount($curTitle);
        $originalTitleWordCount = $this -> wordCount($originalTitle);
    ...
    */
    ...
    private function wordCount($str) {
        $parrten = "/[a-zA-Z]+/";
        preg_match_all($parrten,$str,$arr,PREG_SET_ORDER);
        $englishWorldCount = count($arr);

        $str = preg_replace('/[^\x{4e00}-\x{9fa5}]/u', '', $str);
        $nonEnglishCount = mb_strlen($str);

        return $englishWorldCount + $nonEnglishCount;
    }

    private function removeEnter($str) {
        return trim ($str);
    }
    ...
    public function _cleanConditionally(DOMDocument $article, $tag)
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            if (substr_count($node->getTextContent(), ',') < 10) {
                ...
                $haveToRemove =
                    ($img > 1 && ($p / $img < 0.5 && $p>0 ) && !$node->hasAncestorTag($node, 'figure')) ||
                    (!$isList && $li > $p) ||
                    ($input > floor($p / 3)) ||
    ...
    {
        if ($this->IsNullOrEmptyString($excerpt)) {
            $this->excerpt = null;
        }else {
    ...
    private function printNode($node) {
        if ($node->nodeName==='img') {
            echo "\n<".$node->nodeName." scr=".$node->getAttribute('src').">\n";
            echo "</".$node->nodeName.">\n";
        }
        else {
            echo "\n<".$node->nodeName.">\n";
    ...
    /**
     * Spacial Dash https://stackoverflow.com/questions/10881922/php-preg-replace-regexp-for-dash
     * @param String $str
     * @return string
     */
    private function replaceAllDash ($str) {
        //Replace an N-Dash
    ...
    /**
     * Check if input string is empty, for example ' '
     * @param String $str
     * @return string
     */
