...
namespace App\Library\JiGuang;

use App\Exceptions\JSMSErrorException;
...
class JSMS {
    ...
    private function request($method, $url, $body = []) {
        ...
        if ($httpCode!=200) {
            ...
            $errCode = $bodyArray['error']['code'];
            $message = $bodyArray['error']['message'];
            throw new JSMSErrorException($message, $errCode);
        }
        return $response;
    }
}